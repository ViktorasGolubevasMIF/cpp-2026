...
## Įžanga: tęsiame nuo P06

P06 2 dalyje matėme, kad klaidų valdymas — ne triviali problema. Išbandėme tris būdus (`cerr`, grąžinamoji reikšmė, išimtys) ir pamatėme `throw/try/catch` sintaksę pirmą kartą — `MyString::operator[]` kontekste.

Šioje paskaitoje — pilnas vaizdas:

- Kodėl kiekvienas alternatyvus būdas neišsprendžia **konstruktoriaus problemos**
- Kas nutinka su objektais kai `throw` "šoka" per kelis lygmenis — **stack unwinding**
- `std::exception` hierarchija — kaip ja naudotis sąmoningai
- Kodėl `new` + `throw` = pavojus — ir kas jį išsprendžia

!!! note "P06 ryšys"
    P06 2 dalis baigėsi: *"RAII su Exceptions — vėlesnėse paskaitose"*.
    Tos vėlesnės paskaitos — dabar.
...
!!! abstract "Šios dalies tikslas"
    Greita peržiūra — ką turėjome prieš `throw`.
    Kiekvienas būdas išsprendžia vieną problemą, bet sukuria kitą.
    Kulminacija — **konstruktoriaus kampas**, kurio P06 tik užsiminė.
